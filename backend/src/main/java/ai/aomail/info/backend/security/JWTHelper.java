...
import ai.aomail.info.backend.controllers.LoginRestController;
...
public class JWTHelper {
    ...
    public static String generateToken(AppUser user, String issuer) {
        return JWT.create()
                ...
                .sign(Algorithm.HMAC256(DECODED_SECRET_KEY));
    }
    ...
    public static String extractUsername(String token) {
        return JWT.require(Algorithm.HMAC256(DECODED_SECRET_KEY))
                .build()
                .verify(token)
                .getSubject();
    }
    ...
    public static boolean isTokenExpired(String token) {
        Date expiration = JWT.require(Algorithm.HMAC256(DECODED_SECRET_KEY))
                .build()
                .verify(token)
                .getExpiresAt();
    ...
    private static Claims getAllClaimsFromToken(String token) {
        Key signingKey = Keys.hmacShaKeyFor(DECODED_SECRET_KEY);
